...
def detect_color(h, s, v): #detekterer hvilke farger som plukkes opp
    if s < 40 and v > 200:
        return "Hvit"
    elif s < 40 and v < 200:
        return "Grå"
...
cv2.namedWindow("Tracking") # Lager en trackbar for å kunne stille på Hue, Saturtion og Brightness manuelt for å detektere ønsket farge 
cv2.createTrackbar("LH", "Tracking", 70, 180, nothing)
cv2.createTrackbar("LS", "Tracking", 100, 255, nothing)
cv2.createTrackbar("LV", "Tracking", 100, 255, nothing)
cv2.createTrackbar("UH", "Tracking", 130, 180, nothing)
cv2.createTrackbar("US", "Tracking", 255, 255, nothing)
cv2.createTrackbar("UV", "Tracking", 255, 255, nothing)

while True:
    try: 
        img_resp = urllib.request.urlopen(url) #Laster inn bilde fra kameraet fra IP-adressen som blir opprettet av kameraet
    ...
    mask = cv2.inRange(hsv, l_b, u_b) # Lager et blått filter for å gjenkjenne spesifikke farger, her: blått
